...
//The loop should only work as many as num times before coming back and closing
...
int main(int argc, char** argv)
{
    ...
    if (listen(ls, 5) == SOCKET_ERROR) return 1;
    ...
    cout << "Connected the first socket" << endl;
    ...
    cout << "Connected both" << endl;

    char msg[] = "ready\0";
    int len = strlen(msg); 
    cout << strlen(msg) << " is the length" << endl;
    sendAll(A, msg, len);
    sendAll(B, msg, len); 

    cout << "Sent the message to both" << endl;

    ThreadArgs* first = new ThreadArgs{ A, B };
    ThreadArgs* second = new ThreadArgs{ B, A };
    HANDLE t1 = CreateThread(nullptr, 0, feedbackLoop, first, 0, nullptr);
    HANDLE t2 = CreateThread(nullptr, 0, feedbackLoop, second, 0, nullptr);  
     
    if (t1 != 0) WaitForSingleObject(t1, INFINITE); 
    if (t2 != 0) WaitForSingleObject(t2, INFINITE); 

    if (t1 != 0) CloseHandle(t1);
    if (t2 != 0) CloseHandle(t2);
